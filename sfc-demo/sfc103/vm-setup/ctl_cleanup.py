...
import sys
import os
from ctl_get_env import get_controller_info
from ctl_http_methods import delete
...
if __name__ == "__main__":
    ...
    delete(controller_info['controller_ip'], controller_info[
        'port'], get_service_function_classifiers_uri())
    time.sleep(1)

    print "deleting rendered service path"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_rendered_service_path_uri())
    time.sleep(1)

    print "deleting service function acl"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_service_function_acl_uri())
    time.sleep(1)

    print "deleting service function paths"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_service_function_paths_uri())
    time.sleep(1)

    print "deleting service function metadata"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_service_function_metadata_uri())
    time.sleep(1)

    print "deleting service function chains"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_service_function_chains_uri())
    time.sleep(1)

    print "deleting service function forwarders"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_service_function_forwarders_uri())
    time.sleep(1)

    print "deleting service functions"
    delete(controller_info['controller_ip'], controller_info[
           'port'], get_service_functions_uri())
    time.sleep(1)

    print "deleting service nodes"
    delete(controller_info['controller_ip'],
           controller_info['port'], get_service_nodes_uri())
